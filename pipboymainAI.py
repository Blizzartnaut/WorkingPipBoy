...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # Map update timer (only if GDAL is available and on Windows)
        # Uncomment if map functionality is desired and GDAL is available.
        if self.os_name == "Windows" and gdal is not None:
            self.map_timer = QTimer(self)
            self.map_timer.timeout.connect(self.update_map)
            self.map_timer.start(5000)
        
        # Debug: list available UI attributes to ensure names match what you expect.
        # print("Available UI elements:", dir(self))
    
        # ---------------------------------------------------------------------
        # NEW: Set up QWebEngineView for Leaflet map display.
        self.map_view = QWebEngineView(self)
        # Adjust geometry and placement as needed in your UI.
        # For example, here we set it to a fixed rectangle.
        self.map_view.setGeometry(10, 10, 780, 460)
        # Load the local HTML file (make sure to use an absolute path).
        local_map_path = os.path.abspath("pipboy_map.html")
        self.map_view.load(QUrl.fromLocalFile(local_map_path))
        self.map_view.show()
        # ---------------------------------------------------------------------
        
        # OPTIONAL: Timer for updating GPS marker.
        # Adjust the interval as needed.
        self.gps_timer = QTimer(self)
        self.gps_timer.timeout.connect(self.update_gps_marker)
        self.gps_timer.start(2000)  # Update every 2 seconds
    ...
    def update(self):
        ...
        try:
            ...
            print("UI element missing (DATE or TIME):", e)
